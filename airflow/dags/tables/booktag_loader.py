...
class BookTagObj(BaseModel):
    """
    Data model representing a book-tag relationship with validation.

    Attributes:
        bookid: Foreign key reference to the book
        tagid: Foreign key reference to the tag
    ...
    def from_dict(cls, data: Tuple[Any]) -> "BookTagObj":
        """
        Create BookTagObj from database tuple.

        Args:
            data: Tuple containing (bookid, tagid, updatets)

        Returns:
            BookTagObj: Validated relationship object
        """
        return cls(bookid=data[0], tagid=data[1], updatets=data[2])


class BookTagOriginRepository:
    """
    Repository for retrieving book-tag relationships from PostgreSQL.
    ...
    def list_book_tags(
        ...
        """
        Get batch of book-tag relationships updated after threshold.
        ...
        Returns:
            List[BookTagObj]: Batch of relationship objects
        """
...
class BookTagDestRepository:
    """
    Repository for loading book-tag relationships into ClickHouse.
    Optimized for efficient batch inserts.
    """

    def insert_batch(
        self, conn: ClickhouseClient, relationships: List[BookTagObj]
    ) -> None:
        """
        ...
        data = [[rel.bookid, rel.tagid, rel.updatets] for rel in relationships]
...
class BookTagLoader:
    """
    Orchestrates the complete ETL process for book-tag relationships.
    ...
    def load_book_tags(self) -> None:
        ...
        with self.ch_dest.connection() as conn:
            ...
            last_loaded = conn.execute("SELECT MAX(updatets) FROM BookTag")[0][0]
            if not last_loaded:
            ...
            # Optimize table
            conn.execute("OPTIMIZE TABLE BookTag FINAL")
